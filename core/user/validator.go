...
func (v *validator) validate(
	...
	if !updatedState.Enabled && currentState != nil && currentUserID != nil &&
		...
		v.delegate.Add("enabled", i18n.M(ctx, "user.validation.cannot-disable-self"))
	}

	if request.Password == nil && currentState == nil {
		v.delegate.Add("password", i18n.M(ctx, "common.validation.value-missing"))
	}

	databaseUser, _ := v.repository.FindByUsername(ctx, updatedState.Username)
	if databaseUser != nil && databaseUser.ID != updatedState.ID {
		v.delegate.Add("username", i18n.M(ctx, "user.validation.duplicated-username"))
	}

	if len(updatedState.Username) < minimumUsernameLength {
		v.delegate.Add(
			"username",
			i18n.M(ctx, "common.validation.too-short").V("min", minimumUsernameLength),
		)
	}
	...
	if len(updatedState.Name) < minimumNameLength {
		v.delegate.Add(
			"name",
			i18n.M(ctx, "common.validation.too-short").V("min", minimumNameLength),
		)
	}
...
func (v *validator) validatePermissions(ctx context.Context, permissions Permissions) {
	...
	if permissions.NginxServer == NoAccessAccessLevel {
		v.delegate.Add("permissions.nginxServer", i18n.M(ctx, "user.validation.at-least-read-only"))
	}

	if permissions.Logs == ReadWriteAccessLevel {
		v.delegate.Add("permissions.logs", i18n.M(ctx, "user.validation.cannot-have-write-access"))
	}

	if permissions.ExportData == ReadWriteAccessLevel {
		v.delegate.Add(
			"permissions.exportData",
			i18n.M(ctx, "user.validation.cannot-have-write-access"),
		)
	}
}
...
func (v *validator) validatePermission(ctx context.Context, key string, value AccessLevel) {
	...
	default:
		v.delegate.Add(
			...
			i18n.M(ctx, "user.validation.invalid-access-level"),
		)
	}
}
