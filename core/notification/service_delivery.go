...
func (s *service) processNotificationDeliveries(
	...
	deliverable := deliverableFromNotification(notification)
	for index := range submissions {
		submission := &submissions[index]

		provider := s.findProvider(submission.Provider)
		if provider == nil {
			if err := s.markSubmissionFailed(
				ctx,
				submission,
				"provider not found",
			); err != nil {
				log.Errorf(
					"notification delivery failed for submission %s: %s",
					submission.ID,
					err,
				)
			}
...
func (s *service) markSubmissionFailed(
	ctx context.Context,
	...
	submission.Status = SubmissionStatusFailed
	submission.LastAttemptAt = new(time.Now())
	submission.LastError = new(reason)
	submission.AttemptCount++

	log.Errorf("notification delivery failed for submission %s: %s", submission.ID, reason)
	return s.repository.UpdateProviderSubmission(ctx, submission)
}

func deliverableFromNotification(notification *Notification) Deliverable {
	return Deliverable{
