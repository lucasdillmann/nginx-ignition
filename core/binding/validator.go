...
func (v *validator) validate(
	...
	if net.ParseIP(binding.IP) == nil {
		v.delegate.Add(
			...
			i18n.M(ctx, "binding.validation.invalid-ip"),
		)
	}
	...
	case binding.Type == HTTPBindingType && binding.CertificateID != nil:
		v.delegate.Add(
			certificateIDField,
			i18n.M(ctx, "binding.validation.certificate-id-not-allowed"),
		)
	case binding.Type == HTTPBindingType && binding.CertificateID == nil:
		return nil
	case binding.Type == HTTPSBindingType && binding.CertificateID == nil:
		v.delegate.Add(
			certificateIDField,
			i18n.M(ctx, "binding.validation.certificate-id-required"),
		)
	case binding.Type == HTTPSBindingType:
		...
		if !exists {
			v.delegate.Add(
				certificateIDField,
				i18n.M(ctx, "binding.validation.certificate-id-not-found"),
			)
		}
	default:
		v.delegate.Add(
			fmt.Sprintf("%s[%d].certificateId", pathPrefix, index),
			i18n.M(ctx, "binding.validation.invalid-type"),
		)
	}
