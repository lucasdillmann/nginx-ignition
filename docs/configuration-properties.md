...
| NGINX_IGNITION_NGINX_CONFIG_PATH                         | Path on where the nginx ignition should store the generated nginx's configuration files               | /etc/nginx   | /tmp/nginx-ignition/nginx (`%TEMP%\nginx-ignition\nginx` on Windows) |
| NGINX_IGNITION_VPN_CONFIG_PATH                           | Path on where the nginx ignition should store the generated vpn configuration files                   | /etc/vpn     | /tmp/nginx-ignition/vpn (`%TEMP%\nginx-ignition\vpn` on Windows)     |
...
| NGINX_IGNITION_DATABASE_DATA_PATH                        | Folder on where the database file should be stored. Applicable only for the `sqlite` database.        | /opt/example | /tmp/nginx-ignition/data (`%TEMP%\nginx-ignition\data` on Windows)   |
