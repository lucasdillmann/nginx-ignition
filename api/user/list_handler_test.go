...
func Test_ListHandler_Handle(t *testing.T) {
	...
	t.Run("returns 200 OK with user list on success", func(t *testing.T) {
		...
		page := &corepagination.Page[user.User]{
			Contents: mockUsers,
		}
		...
		r := gin.New()
		r.GET("/api/users", handler.handle)

		w := httptest.NewRecorder()
		req := httptest.NewRequest("GET", "/api/users?pageSize=10&pageNumber=1", nil)
		r.ServeHTTP(w, req)

		assert.Equal(t, http.StatusOK, w.Code)
		var resp pagination.PageDTO[userResponseDTO]
