...
func Test_StartHandler_Handle(t *testing.T) {
	...
	t.Run("returns 204 No Content on success", func(t *testing.T) {
		...
		handler := startHandler{commands}
		r := gin.New()
		r.POST("/api/nginx/start", handler.handle)

		w := httptest.NewRecorder()
		req := httptest.NewRequest("POST", "/api/nginx/start", nil)
		r.ServeHTTP(w, req)
