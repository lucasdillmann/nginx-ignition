...
func Test_markAsReadHandler(t *testing.T) {
	t.Run("handle", func(t *testing.T) {
		...
		t.Run("returns 401 Unauthorized without subject", func(t *testing.T) {
			handler := markAsReadHandler{commands: nil}
			engine := gin.New()
			engine.POST("/:id/mark-as-read", handler.handle)
