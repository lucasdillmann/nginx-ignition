...
func Test_UpdateHandler_Handle(t *testing.T) {
	...
	t.Run("returns 204 No Content on success", func(t *testing.T) {
		...
		router := gin.New()
		router.PUT("/api/access-lists/:id", updateHandler{commands}.handle)

		w := httptest.NewRecorder()
		req := httptest.NewRequest("PUT", "/api/access-lists/"+id.String(), bytes.NewBuffer(body))
	...
	t.Run("returns 404 Not Found when ID is invalid", func(t *testing.T) {
		router := gin.New()
		router.PUT("/api/access-lists/:id", updateHandler{nil}.handle)

		w := httptest.NewRecorder()
		req := httptest.NewRequest("PUT", "/api/access-lists/invalid-uuid", bytes.NewBuffer(body))
		req.Header.Set("Content-Type", "application/json")
		router.ServeHTTP(w, req)
