...
func Test_deleteHandler(t *testing.T) {
	t.Run("handle", func(t *testing.T) {
		t.Run("returns 204 No Content on success", func(t *testing.T) {
			...
			engine := gin.New()
			engine.DELETE("/api/streams/:id", handler.handle)

			recorder := httptest.NewRecorder()
			request := httptest.NewRequest("DELETE", "/api/streams/"+id.String(), nil)
		...
		t.Run("returns 404 Not Found on invalid ID", func(t *testing.T) {
			...
			recorder := httptest.NewRecorder()
			request := httptest.NewRequest("DELETE", "/api/streams/invalid", nil)
			engine.ServeHTTP(recorder, request)
		...
		t.Run("panics on command error", func(t *testing.T) {
			...
			engine := gin.New()
			engine.DELETE("/api/streams/:id", func(ginContext *gin.Context) {
				defer func() {
					if r := recover(); r != nil {
