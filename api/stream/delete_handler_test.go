...
func Test_DeleteHandler(t *testing.T) {
	...
	t.Run("Handle", func(t *testing.T) {
		t.Run("returns 204 No Content on success", func(t *testing.T) {
			...
			r := gin.New()
			r.DELETE("/api/streams/:id", handler.handle)

			w := httptest.NewRecorder()
			req := httptest.NewRequest("DELETE", "/api/streams/"+id.String(), nil)
			r.ServeHTTP(w, req)
		...
		t.Run("returns 404 Not Found on invalid ID", func(t *testing.T) {
			...
			w := httptest.NewRecorder()
			req := httptest.NewRequest("DELETE", "/api/streams/invalid", nil)
			r.ServeHTTP(w, req)
		...
		t.Run("panics on command error", func(t *testing.T) {
			...
			r := gin.New()
			r.DELETE("/api/streams/:id", func(c *gin.Context) {
				defer func() {
					if r := recover(); r != nil {
