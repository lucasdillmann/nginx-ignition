...
func New(database *database.Database) server.Repository {
	return repository{
		database: database,
	}
}

func (r repository) FindByID(ctx context.Context, id uuid.UUID) (*server.Certificate, error) {
...
func (r repository) Save(ctx context.Context, certificate *server.Certificate) error {
	transaction, err := r.database.Begin()
	if err != nil {
		return err
	}
...
func (r repository) FindPage(
	...
	searchTerms *string,
) (*pagination.Page[*server.Certificate], error) {
...
func (r repository) FindAllDueToRenew(ctx context.Context) ([]*server.Certificate, error) {
	var certificates []certificateModel
