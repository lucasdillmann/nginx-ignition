...
interface CertificateIssuePageState {
    ...
    formValues: IssueServerCertificateRequest
    error?: Error
}

export default class ServerCertificateIssuePage extends React.Component<unknown, CertificateIssuePageState> {
    ...
    private async submit() {
        ...
        this.saveModal.show(
            "Hang on tight",
            "We're issuing your server certificate. This can take several seconds, feel free to grab a cup of coffee.",
        )
        this.setState({ validationResult: new ValidationResult() })

        const certificateRequest: IssueServerCertificateRequest = {
            ...formValues,
    ...
    private handleResponse(response: IssueServerCertificateResponse) {
        ...
        if (success) {
            ...
            Notification.error(
                "Issue failed",
                errorReason ?? "Unable to issue the server certificate at this moment. Please try again later.",
            )
        }
    }
    ...
    private renderForm() {
        ...
        return (
            <Form<IssueServerCertificateRequest>
    ...
    private updateShellConfig(enableActions: boolean) {
        AppShellContext.get().updateConfig({
            title: "New server certificate",
            subtitle: "Issue or upload a server certificate for use with the nginx's virtual hosts",
            actions: [
                {
