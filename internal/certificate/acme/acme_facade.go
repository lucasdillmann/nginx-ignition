...
func issueCertificate(
	...
	caURL, preferredChain := resolveCA(parameters)
	...
	if preferredChain != nil {
		request.PreferredChain = *preferredChain
	}
...
func resolveCA(parameters map[string]any) (string, *string) {
	ca := parameters[certificateAuthorityFieldID]

	switch ca {
	case ActalisProduction:
		return lego.DirectoryURLActalis, nil
	case ZeroSSLProduction:
		return lego.DirectoryURLZeroSSL, nil
	case LetsEncryptSandbox:
		return lego.DirectoryURLLetsEncryptStaging, nil
	default:
		return lego.DirectoryURLLetsEncrypt, new("ISRG Root X1")
	}
}
