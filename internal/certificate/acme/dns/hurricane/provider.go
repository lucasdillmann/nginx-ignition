...
func (p *Provider) Name(ctx context.Context) *i18n.Message {
	return i18n.M(ctx, i18n.K.CertificateAcmeDnsHurricaneName)
}
...
func (p *Provider) DynamicFields(ctx context.Context) []dynamicfields.DynamicField {
	return dns.LinkedToProvider(p.ID(), []dynamicfields.DynamicField{
		{
			ID:          tokensFieldID,
			Description: i18n.M(ctx, i18n.K.CertificateAcmeDnsHurricaneTokens),
			HelpText:    i18n.M(ctx, i18n.K.CertificateAcmeDnsHurricaneTokensHelp),
			Required:    true,
			Sensitive:   true,
	...
	for _, pair := range pairs {
		...
		if len(parts) != 2 {
			return nil, coreerror.New(
				i18n.M(
					ctx,
					i18n.K.CertificateAcmeDnsHurricaneErrorHurricaneInvalidTokenFormat,
				),
				true,
			)
		}
