...
func (p *Provider) Name(ctx context.Context) *i18n.Message {
	return i18n.M(ctx, i18n.K.CertificateAcmeDnsManageengineName)
}
...
func (p *Provider) DynamicFields(ctx context.Context) []dynamicfields.DynamicField {
	return dns.LinkedToProvider(p.ID(), []dynamicfields.DynamicField{
		{
			ID:          clientIDFieldID,
			Description: i18n.M(ctx, i18n.K.CertificateAcmeDnsManageengineClientId),
			Required:    true,
		...
		},
		{
			...
			Description: i18n.M(
				ctx,
				i18n.K.CertificateAcmeDnsManageengineClientSecret,
			),
			Required:  true,
			Sensitive: true,
			Type:      dynamicfields.SingleLineTextType,
