...
func (p *Provider) Name(ctx context.Context) *i18n.Message {
	return i18n.M(ctx, i18n.K.CertificateAcmeDnsDnshomedeName)
}
	...
	return dns.LinkedToProvider(p.ID(), []dynamicfields.DynamicField{
		{
			...
			Description: i18n.M(ctx, i18n.K.CertificateAcmeDnsDnshomedeCredentials),
			HelpText: i18n.M(
				ctx,
				i18n.K.CertificateAcmeDnsDnshomedeCredentialsHelp,
			),
			Required:  true,
			Sensitive: true,
	...
	for _, pair := range pairs {
		...
		if len(parts) != 2 {
			return nil, coreerror.New(
				i18n.M(
					ctx,
					i18n.K.CertificateAcmeDnsDnshomedeErrorDnshomedeInvalidCredentialsFormat,
				),
				true,
			)
		}
