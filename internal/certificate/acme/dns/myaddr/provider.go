...
func (p *Provider) Name(ctx context.Context) *i18n.Message {
	return i18n.M(ctx, i18n.K.CertificateAcmeDnsMyaddrName)
}
	...
	return dns.LinkedToProvider(p.ID(), []dynamicfields.DynamicField{
		{
			...
			Description: i18n.M(
				ctx,
				i18n.K.CertificateAcmeDnsMyaddrPrivateKeysMapping,
			),
			HelpText: i18n.M(
				ctx,
				i18n.K.CertificateAcmeDnsMyaddrPrivateKeysMappingHelp,
			),
			Required:  true,
			Sensitive: true,
	...
	for _, pair := range pairs {
		...
		if len(parts) != 2 {
			return nil, coreerror.New(
				i18n.M(
					ctx,
					i18n.K.CertificateAcmeDnsMyaddrErrorMyaddrInvalidCredentialsFormat,
				),
				true,
			)
		}
